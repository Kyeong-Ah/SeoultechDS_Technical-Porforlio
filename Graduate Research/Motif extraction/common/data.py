...
def load_dataset(name):
    ...
    # (NEW) ZAP GraphML 파일 직접 읽기
    elif name.startswith("zap-graphml:"):
        path = name.split(":", 1)[1]
        graph = nx.read_graphml(path)           # 보통 DiGraph로 로딩됩니다
        # 혹시 MultiDiGraph로 올 경우 단순화 원하면 다음 주석 해제:
        # graph = nx.DiGraph(graph)
        train = [graph]
        test  = [graph]                          # 하나의 대형 그래프를 학습/검증에 함께 사용
        task = "graph"
    ...
    if task == "graph":
        ...
        for i, graph in tqdm(enumerate(dataset)):
            if not type(graph) == nx.Graph:
                ...
                graph = pyg_utils.to_networkx(graph).to_undirected() # 기존 코드. 무향 변환하는 코드임.
                graph = pyg_utils.to_networkx(graph) # 방향성 유지하도록 수정.
            if i < train_len:
                train.append(graph)
            else:
                test.append(graph)
...
class DiskDataSource(DataSource):
    ...
    def gen_batch(self, a, b, c, train, max_size=15, min_size=5, seed=None,
        ...
        while len(neg_a) < batch_size // 2:
            ...
            # if filter_negs: # 기존의 무향 전제 코드
            #     matcher = nx.algorithms.isomorphism.GraphMatcher(neigh_a, neigh_b)
            #     if matcher.subgraph_is_isomorphic(): # a <= b (b is subgraph of a)
            #         continue
            # ↓↓ 방향성 고려하는 수정된 코드
            if filter_negs:
                # 양쪽이 DiGraph면 방향성까지 동일해야 '동형'으로 간주하여 필터
                # (그렇지 않으면 기존대로 무향 GraphMatcher 사용)
                GM = iso.DiGraphMatcher if (neigh_a.is_directed() and neigh_b.is_directed()) else iso.GraphMatcher
                matcher = GM(neigh_a, neigh_b)

                # subgraph_is_isomorphic()는 "첫 번째 인자(neigh_a)가 두 번째(neigh_b)를 부분그래프로 포함하는지" 검사
                # 즉, neigh_b ≤ neigh_a 이면 True 입니다. (원래 주석의 a<=b는 반대였음)
                if matcher.subgraph_is_isomorphic():
                    continue
            # ↑↑ 방향성 고려하는 수정된 코드
