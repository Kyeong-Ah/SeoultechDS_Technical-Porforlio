...
# def make_data_source(args): # 기존 make_data_source 함수
...
def make_data_source(args): # zap 그래프 사용에 맞게 수정된 함수
    ds = args.dataset
    # 한 번만 분할해서 앞(head)/뒤(tail)로 나눔
    parts = ds.split("-", 1)
    head = parts[0]
    tail = parts[1] if len(parts) > 1 else None

    if head == "syn":
        if tail is None or tail == "balanced":
            data_source = data.OTFSynDataSource(node_anchored=args.node_anchored)
        elif tail == "imbalanced":
            data_source = data.OTFSynImbalancedDataSource(node_anchored=args.node_anchored)
        else:
            raise Exception("Error: unrecognized dataset")
    else:
        # 여기부터는 디스크 기반(실데이터) 로더
        # 1) 명시적으로 'imbalanced'면 기존 로직 유지
        if tail == "imbalanced":
            data_source = data.DiskImbalancedDataSource(head, node_anchored=args.node_anchored)
        # 2) tail이 없거나 'balanced'면 기존처럼 head만 넘김
        elif tail is None or tail == "balanced":
            data_source = data.DiskDataSource(head, node_anchored=args.node_anchored)
        # 3) 그 외(예: 'graphml:/...')는 전체 문자열을 그대로 넘겨 커스텀 로더에서 처리
        else:
            data_source = data.DiskDataSource(ds, node_anchored=args.node_anchored)
