...
def _dir_neighbors(G, u): # 9/20 추가
...
class MCTSSearchAgent(SearchAgent):
    ...
    def has_min_reachable_nodes(self, graph, start_node, n):
        for depth_limit in range(n+1):
            # edges = nx.bfs_edges(graph, start_node, depth_limit=depth_limit) # 기존 코드
            edges = nx.bfs_edges(graph.to_undirected(), start_node, depth_limit=depth_limit) # 수정된 코드. 도달 가능성 체크는 무향으로 수정.
            # ↑↑ 방향 그래프에선 out-degree 0일 수 있으니, “씨앗” 배제용 리치 체크는 무향 변환으로만 설정.
    ...
    def step(self):
        ...
        for simulation_n in tqdm(range(self.n_trials //
            ...
            neigh = [start_node]
            # frontier = list(set(graph.neighbors(start_node)) - set(neigh)) # 기존 코드
            # frontier = list(set(utils.neighbors_dir(graph, start_node)) - set(neigh)) # neighbors()를 neighbors_dir()로 수정     # 이건 9월 이전 수정했던 코드
            frontier = list(set(_dir_neighbors(graph, start_node)) - set(neigh)) # neighbors_dir()로 수정했던 걸 _dir_neighbor로 수정
            ...
            while frontier and len(neigh) < self.max_size:
                '''cand_neighs, anchors = [], [] # 기존 cand_neighs, anchors 생성 루프
                ...
                cand_neighs, anchors, frontier_valid = [], [], []
                for cand_node in frontier:
                    cand_neigh = graph.subgraph(neigh + [cand_node]).copy()
                    cand_neigh.remove_edges_from(nx.selfloop_edges(cand_neigh))
                    # ✨ 선형 체인 필터
                    if not utils.is_linear_path(cand_neigh):
                        continue
                    cand_neighs.append(cand_neigh)
                    frontier_valid.append(cand_node)
                    if self.node_anchored:
                        anchors.append(neigh[0])
                # 유효 후보가 하나도 없으면 이 씨앗 확장은 종료
                if not cand_neighs:
                    break
                ...
                for cand_node, cand_emb in zip(frontier_valid, cand_embs):   # 9/20 frontier_valid로 수정
                    score, n_embs = 0, 0
                    ...
                    if node_score > best_node_score:
                        ...
                        best_node = cand_node
                # frontier = list(((set(frontier) | set(graph.neighbors(best_node))) - visited) - set([best_node])) # 기존 코드
                # frontier = list(((set(frontier) | set(utils.neighbors_dir(graph, best_node))) - visited) - set([best_node])) # neighbors_dir()로 수정된 코드 
                frontier = list(((set(frontier) | set(_dir_neighbors(graph, best_node))) - visited) - set([best_node])) # 9월 이후에 또 수정한 코드
    ...
    def finish_search(self):
        ...
        for pattern_size in range(self.min_pattern_size, self.max_pattern_size+1):
            '''for wl_hash, count in sorted(counts[pattern_size].items(), key=lambda  # 이전코드
                x: x[1], reverse=True)[:self.out_batch_size]:
                cand_patterns_uniq.append(random.choice(
                    self.wl_hash_to_graphs[wl_hash]))'''
            items = sorted(counts[pattern_size].items(), key=lambda x: x[1], reverse=True)  # 포함 이하 5줄이 9/20 수정된 코드
            if self.out_batch_size > 0:
                items = items[:self.out_batch_size]
            for wl_hash, count in items:
...
class GreedySearchAgent(SearchAgent):
    ...
    def init_search(self):
        ...
        for trial in range(self.n_trials):
            ...
            neigh = [start_node]
            # frontier = list(set(graph.neighbors(start_node)) - set(neigh)) # 기존 코드
            # frontier = list(set(utils.neighbors_dir(graph, start_node)) - set(neigh)) # neighbors() -> neighbors_dir()로 수정된 코드
            frontier = list(set(_dir_neighbors(graph, start_node)) - set(neigh)) # _dir_neighrbos()로 수정된 코드
            visited = set([start_node])
    ...
    def step(self):
        ...
        for beam_set in tqdm(self.beam_sets):
            ...
            for _, neigh, frontier, visited, graph_idx in beam_set:
                ...
                '''cand_neighs, anchors = [], []   #기존 cand_neighs, anchors 생성 코드
                for cand_node in frontier:
                    cand_neigh = graph.subgraph(neigh + [cand_node])
                    cand_neighs.append(cand_neigh)
                    if self.node_anchored:
                ...
                cand_neighs, anchors, frontier_valid = [], [], []   # 9/20 수정된 코드
                for cand_node in frontier:
                    cand_neigh = graph.subgraph(neigh + [cand_node]).copy()
                    cand_neigh.remove_edges_from(nx.selfloop_edges(cand_neigh))
                    # ✨ 선형 체인 필터
                ...
                for cand_node, cand_emb in zip(frontier, cand_embs):
                    ...
                    if score < best_score:
                        best_score = score
                        best_node = cand_node
                    # new_frontier = list(((set(frontier) | set(graph.neighbors(cand_node))) - visited) - set([cand_node])) # 기존 코드
                    # new_frontier = list(((set(frontier) | set(utils.neighbors_dir(graph, cand_node))) - visited) - set([cand_node])) # 수정된 코드
                    new_frontier = list(((set(frontier) | set(_dir_neighbors(graph, cand_node))) - visited) - set([cand_node])) # 9/20 수정된 코드
                    new_beams.append((
...
def finish_search(self):   # 9/20 수정된 코드
    if self.analyze:
        ...
        if cur_rank_method == "margin":
            ...
            for pattern in sorted(cands, key=lambda x: x[0]):
                ...
                if wl_hash not in wl_hashes:
                    ...
                    if self.out_batch_size > 0 and len(cand_patterns_uniq_size) >= self.out_batch_size:
                        break
            cand_patterns_uniq += cand_patterns_uniq_size

        elif cur_rank_method == "counts":
            items = list(sorted(self.counts[pattern_size].items(),
                                key=lambda x: len(x[1]), reverse=True))
            if self.out_batch_size > 0:
                items = items[:self.out_batch_size]
            for _, neighs in items:
