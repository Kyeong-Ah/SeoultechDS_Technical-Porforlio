...
# === Add ===
# 이미 NX면 그대로, PyG(Data/HeteroData)일 때만 to_networkx
def _ensure_nx(g):
    if isinstance(g, (nx.Graph, nx.DiGraph)):
        return g
    if isinstance(g, (nx.MultiGraph, nx.MultiDiGraph)):
        # 병렬 엣지가 있으면 단순화 (원치 않으면 이 줄 지우세요)
        return nx.DiGraph(g) if g.is_directed() else nx.Graph(g)
    return pyg_utils.to_networkx(g)
...
#                 frontier = list(set(graph.neighbors(start_node)) - set(neigh)) # 기존 코드
                frontier = list(set(utils.neighbors_dir(graph, start_node)) - set(neigh)) # 수정된 코드
                while len(neigh) < len(query) and frontier:
                    new_node = random.choice(list(frontier))
                    assert new_node not in neigh
                    neigh.append(new_node)
#                    frontier += list(graph.neighbors(new_node)) # 기존 코드
                    frontier += list(utils.neighbors_dir(graph, new_node)) # 수정된 코드
...
def count_graphlets_helper(inp):
    ...
    # ↓↓ ★ 추가된 함수. matcher 선택 헬퍼: 양쪽이 DiGraph면 DiGraphMatcher, 아니면 GraphMatcher
    def make_matcher(T, Q, node_match=None):
        GM = iso.DiGraphMatcher if (T.is_directed() and Q.is_directed()) else iso.GraphMatcher
        return GM(T, Q, node_match=node_match) if node_match is not None else GM(T, Q)
    # ★ 앵커와 랭크 동시 비교 위한 코드
    def node_match(a, b):
        # 둘 다 없으면 0으로 가정
        return (a.get('anchor', 0) == b.get('anchor', 0)) and (a.get('rank', -1) == b.get('rank', -1))
    ...
    if method == "bin":
        if node_anchored:
            for anchor in (target.nodes if anchor_or_none is None else [anchor_or_none]):
                ...
                node_match = iso.categorical_node_match(['anchor'], [0]) # 추가된 코드
#                 matcher = iso.GraphMatcher(target, query, node_match=iso.categorical_node_match(["anchor"], [0])) # 기존 코드
                matcher = make_matcher(target, query, node_match=node_match) # 수정된 코드
...
#            matcher = iso.GraphMatcher(target, query) # 기존 코드
            matcher = make_matcher(target, query) # 수정된 코드
            n += int(matcher.subgraph_is_isomorphic())
    elif method == "freq":
#         matcher = iso.GraphMatcher(target, query) # 기존 코드
        matcher = make_matcher(target, query) # 수정된 코드
        # 주의: subgraph_isomorphisms_iter()는 DiGraphMatcher에서도 동작하며 방향을 반영합니다.
...
if __name__ == "__main__":
    ...
    if args.dataset == 'enzymes':
        ...
        dataset = TUDataset(root='/tmp/ENZYMES', name='ENZYMES')
    elif args.dataset.startswith('zap-graphml:'): # zap 데이터용 분기 추가
        gpath = args.dataset.split(':', 1)[1]
        G = nx.read_graphml(gpath)            # 방향 그래프 유지
        if isinstance(G, nx.MultiDiGraph):    # (선택) 병렬 엣지 단순화
            G = nx.DiGraph(G)
        dataset = [G]


    targets = []
#     for i in range(len(dataset)): # 기존에 사용하던 타겟 그래프 수집 코드
#         graph = dataset[i]
#         if not type(graph) == nx.Graph:
# #             graph = pyg_utils.to_networkx(dataset[i]).to_undirected() # 기존 코드. 무방향성 전제
#             graph = pyg_utils.to_networkx(dataset[i]) # 수정된 코드. 방향성 전제.
#         targets.append(graph)
    for g in dataset: # 수정된 코드. 이거 두 줄로 원래 코드 대체한 것.
        targets.append(_ensure_nx(g))
